...
int Delta_S_Line(S_Line_Fix_Type *S_Line_Fix,POSE_DECARE_Type* Decare_Point,S_Line_J_Runin *S_Line_Runin)
{
...
*/	
	...
	Vc1 = 1200*S_Line_Fix->speed_rate*S_Line_Fix->limit_rate;														//最大速度       J1
	Amax1 = 6000*S_Line_Fix->speed_rate*S_Line_Fix->speed_rate*S_Line_Fix->limit_rate;								//最大加速度
	...
	Vc2 = 1200*S_Line_Fix->speed_rate*S_Line_Fix->limit_rate;														//最大速度       J2
	Amax2 = 6000*S_Line_Fix->speed_rate*S_Line_Fix->speed_rate*S_Line_Fix->limit_rate;								//最大加速度
	...
	Vc3 = 1200*S_Line_Fix->speed_rate*S_Line_Fix->limit_rate;														//最大速度       J3
	Amax3 = 6000*S_Line_Fix->speed_rate*S_Line_Fix->speed_rate*S_Line_Fix->limit_rate;								//最大加速度
	...
	Vc4 = 4000*S_Line_Fix->speed_rate*S_Line_Fix->limit_rate;														//最大速度       J4
	Amax4 = 20000*S_Line_Fix->speed_rate*S_Line_Fix->speed_rate*S_Line_Fix->limit_rate;								//最大加速度
	...
	if (Vs>=0 && Vs<Vc && Ve >= 0 && Ve < Vc)
	{
		...
		if (Vs<Vc-Amax*tm && Ve< Vc-Amax*tm )
		{
			...
			if (L==fabsf(L1))
			{
			    if(L!=0)
				{
					if (L>((1.1434*pow((S_Line_Fix->speed_rate*100),6))/(J*J)))
					{					
						J=(pow((1.1434*pow((S_Line_Fix->speed_rate*100),6))*1.0/L,0.5));																																																				//(0.06008/L)^(1/2);     //J=((0.06008*（百分比*100）的六次方)/L)^(1/2)    0.0601
						t1=Amax/J;
						t2=(pow((Amax*(Amax*Amax*Amax + 4*L*J*J)),0.5) - 3*Amax*Amax)/(2*Amax*J);    
					}	
					else if (L<((1.1434*pow((S_Line_Fix->speed_rate*100),6))/(J*J)))
					{
						J = J;
						t1 = (0.5742*pow(L,0.3069))/(S_Line_Fix->speed_rate*100);									//根据L和t1的曲线关系进行求解
						//S=L-S7求解t2
						t2=(500000*pow(((J*pow(L,(3069.0/5000))*t1*(8242641*J*t1 + 1000000000000*pow(L,(1931.0/5000))*S_Line_Fix->speed_rate*S_Line_Fix->speed_rate))/250000000000),0.5) - 8613*J*pow(L,(3069.0/10000))*t1)/(1000000*J*S_Line_Fix->speed_rate*t1);
					}
					J1=J;
				}
			}
...
*/
			...
			else
			{
				Amax3 = J3 * t1;
				Amax4 = J4 * t1;
				if(Amax3 > 6000*S_Line_Fix->speed_rate*S_Line_Fix->speed_rate*S_Line_Fix->limit_rate ||Amax3 < -6000*S_Line_Fix->speed_rate*S_Line_Fix->speed_rate*S_Line_Fix->limit_rate
				   ||Amax4 > 20000*S_Line_Fix->speed_rate*S_Line_Fix->speed_rate*S_Line_Fix->limit_rate ||Amax4 < -20000*S_Line_Fix->speed_rate*S_Line_Fix->speed_rate*S_Line_Fix->limit_rate )
				{
				    Amax3 = 6000*S_Line_Fix->speed_rate*S_Line_Fix->speed_rate*S_Line_Fix->limit_rate;
				    Amax4 = 20000*S_Line_Fix->speed_rate*S_Line_Fix->speed_rate*S_Line_Fix->limit_rate;
				    L = max2;
					if (L==fabsf(L3))
					{
						Vc=Vc3;
...
/*				if (L==fabsf(L1))
			...
			{
				if (L>((42.198*pow((S_Line_Fix->speed_rate*100),6))/(J*J)))
				{					
					J=(pow((42.198*pow((S_Line_Fix->speed_rate*100),6))*1.0/L,0.5));																																																				//(0.06008/L)^(1/2);     //J=((0.06008*（百分比*100）的六次方)/L)^(1/2)    0.0601
					t1=Amax/J;
					t2=(pow((Amax*(Amax*Amax*Amax + 4*L*J*J)),0.5) - 3*Amax*Amax)/(2*Amax*J);    
				}	
				else if (L<((42.198*pow((S_Line_Fix->speed_rate*100),6))/(J*J)))
				{
					J = J;
					t1 = (0.5915*pow(L,0.3173))/(S_Line_Fix->speed_rate*100);									//根据L和t1的曲线关系进行求解
					//S=L-S7求解t2
					t2=(200000*pow(((J*pow(L,(3173.0/5000))*t1*(1399489*J*t1 + 160000000000*pow(L,(1827.0/5000))*S_Line_Fix->speed_rate*S_Line_Fix->speed_rate))/40000000000),0.5) - 3549*J*pow(L,(3173.0/10000))*t1)/(400000*J*S_Line_Fix->speed_rate*t1);
				}
				J4=J;
			}
